...
<script>
  import {getAdoptDetail,queryAllUserInfo} from '@/api/interactive'
  ...
  export default {
    ...
    methods: {
      fetchData() {//页面初始化加载数据
        ...
        queryAllUserInfo(id,0,0).then(response => {
          console.log("进入数据")
          console.log(response)
          that.userId = response.data.id
          that.commentCount = response.data.commentCount
          that.headPortraitUrl =  response.data.headPortraitUrl
          that.wxname = response.data.wxname
          that.telephone =  response.data.telephone
          that.userRemark = response.data.userRemark
          that.followCount =  response.data.followCount
          that.fansCount =  response.data.fansCount
          that.favoriteCount = response.data.favoriteCount
          that.lastLoginTime = response.data.lastLoginTime
          that.replayCount = response.data.replayCount
          that.collectionCount = response.data.collectionCount
        })
      }
    }
  }
