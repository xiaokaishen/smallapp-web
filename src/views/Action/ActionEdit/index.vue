...
<!--      <el-form-item label="艺术品所属类别" prop="customArtworkType">
      ...
      <el-form-item label="活动简介" class="unneed">
        <el-input type="textarea" :autosize="{ minRows: 5 }" class="form-size"
                  v-model="customArtWorkForm.activityIntroduction"></el-input>
      </el-form-item>
...
<script>
  import {addCustomArtwork, getDetailById, updateCustomerArtWork,addActivity,queryActivity,updateActivity,delActivity} from '@/api/customArtwork'
    ...
    data() {
      return {
        ...
        rules: {                      //表单校验规则
         ...
         /* activityBeginData: [
        {type:string,required: true, message: '请填写艺术品介绍信息', trigger: 'change'}
         ],*/
          customArtworkRemark: [
     ...
     //   this.listLoading = true;
        queryActivity(activityId,0, 0).then(response => {
          if(response.status == 200) {
            ...
            let data = response.data.queryActivity;
            data.activityMaxNumber = data.activityMaxNumber+"";
            self.customArtWorkForm = data;
          }
      ...
      submitForm() {
        ...
        this.$refs['customArtWorkForm'].validate((valid) => {
          ...
          if (valid) {//验证通过
            ...
            updateActivity(this.customArtWorkForm).then(response => {
